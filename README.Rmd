...
* To avoid namespace collisions, it is recommended to not load the package, but instead refer to each function with the package name (e.g. `ggscribe::sec_axis_text()`.
* `sec_axis_text` adjusts space in the plot, whereas `axis_*` functions do not.
...
* Where you require annotation text along a axis with different angles etc, use a combination of `sec_axis_text` and `axis_*` functions. The `sec_axis_text` function should include the annotation that requires the maximum space that you want the plot to adjust to. 
