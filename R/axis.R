...
#' #' @param ticks_length Total tick length as a grid unit. Supports `rel()` to
...
#'     ticks_length <- calculate_default_length()
#'   } else if (inherits(ticks_length, "rel")) {
...
#'   flip_direction <- FALSE
#'   if (is.null(ticks_length)) {
#'     ticks_length <- calculate_default_ticks_length()
#'   } else if (inherits(ticks_length, "rel")) {
#'     rel_value      <- as.numeric(ticks_length)
...
#' #' The bar inherits style from `axis.line` in the set theme (same rendering path
#' #' as [annotate_axis_line()]); the end caps inherit from `axis.ticks` (same
#' #' rendering path as [annotate_axis_ticks()]). Requires
#' #' `coord_cartesian(clip = "off")`.
#' #'
#' #' @param ... Not used. Forces named arguments.
#' #' @param position One of `"top"`, `"bottom"`, `"left"`, or `"right"`. Inferred
...
#' #' @param colour Colour for both bar and caps. Inherits from `axis.ticks` in
#' #'   the set theme.
#' #' @param linewidth Line width for both bar and caps. Inherits from `axis.ticks`
#' #'   in the set theme. Supports `rel()`.
#' #' @param linetype Inherits from `axis.ticks` in the set theme.
#' #' @param ticks_length Length of the bracket end caps as a grid unit. Supports
#' #'   `rel()`. Defaults to the theme tick length.
#' #' @param elements_to One of `"keep"`, `"transparent"`, or `"blank"`. Controls
#' #'   whether native theme axis line and ticks are suppressed. Defaults to
#' #'   `"transparent"`.
#' #'
#' #' @return A list of ggplot2 annotation layers and theme elements.
...
#'   }
#'
#'   cap_length <- if (is.null(ticks_length)) {
#'     calculate_default_length()
#'   } else if (inherits(ticks_length, "rel")) {
#'     grid::unit(as.numeric(ticks_length) * as.numeric(grid::convertUnit(calculate_default_length(), "pt")), "pt")
#'   } else if (inherits(ticks_length, "unit")) {
#'     ticks_length
#'   } else {
#'     grid::unit(abs(ticks_length), "pt")
#'   }
#'
#'   stamp <- list()
#'
#'   # ---- Suppress Original Theme Elements -------------------------------------
#'
#'   if (elements_to != "keep") {
#'     theme_mod   <- list()
...
#'
#'   # ---- Bar ------------------------------------------------------------------
#'   # Uses annotate("segment") — same rendering path as annotate_axis_line() so
#'   # the bar is visually identical to a native axis line.
#'
#'   if (axis == "x") {
#'     stamp <- c(stamp, list(ggplot2::annotate(
#'       "segment",
...
#'
#'   # ---- End Caps -------------------------------------------------------------
#'   # Uses annotation_custom + segmentsGrob pinned to bracket endpoints — same
#'   # rendering path as annotate_axis_ticks().
#'
...
#'       x0 = grid::unit(0.5, "npc"), x1 = grid::unit(0.5, "npc"),
#'       y0 = grid::unit(1, "npc"),
#'       y1 = grid::unit(1, "npc") + cap_length,
#'       gp = gp_cap
#'     )
#'   } else if (position == "left") {
#'     grid::segmentsGrob(
#'       x0 = grid::unit(0, "npc"),
#'       x1 = grid::unit(0, "npc") - cap_length,
#'       y0 = grid::unit(0.5, "npc"), y1 = grid::unit(0.5, "npc"),
#'       gp = gp_cap
#'     )
#'   } else {
#'     grid::segmentsGrob(
#'       x0 = grid::unit(1, "npc"),
#'       x1 = grid::unit(1, "npc") + cap_length,
#'       y0 = grid::unit(0.5, "npc"), y1 = grid::unit(0.5, "npc"),
#'       gp = gp_cap
#'     )
#'   }
#'
#'   # Cap drawn at every break — bar spans min to max, caps at each break.
#'
