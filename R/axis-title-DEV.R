...
annotate_axis_title <- function(
  ...
  # ---- Infer Position & Axis ------------------------------------------------
  if (is.null(position)) {
    rlang::abort("Must specify `position` for `annotate_axis_title()`.")
  } else {
  ...
  if (is.null(offset)) {
    ...
    # For left/right: the offset dimension is label WIDTH, not height.
    # Font size is a good proxy for top/bottom (height ≈ font size),
    # but for left/right we need to measure a representative string.
    axis_text_dim_pts <- if (position %in% c("left", "right")) {
      rep_grob <- grid::textGrob(
        "00.00",
        gp = grid::gpar(
          fontsize   = axis_text_size_pts,
          fontfamily = resolved_text_element$family %||% ""
        )
      )
      as.numeric(grid::convertUnit(grid::grobWidth(rep_grob), "pt"))
    } else {
      axis_text_size_pts
    }
    ...
    inner_margin_index <- switch(position, bottom = 1L, top = 3L, left = 2L, right = 4L)
    outer_margin_index <- switch(position, bottom = 3L, top = 1L, left = 4L, right = 2L)
    ...
    axis_text_outer_margin_pts <- 0
    if (!is.null(resolved_text_element$margin)) {
      m <- resolved_text_element$margin
      if (inherits(m, c("margin", "unit")) && length(m) >= outer_margin_index) {
        axis_text_outer_margin_pts <- as.numeric(
          grid::convertUnit(m[outer_margin_index], "pt")
        )
      }
    }

    offset_pts <- tick_length_pts + axis_text_margin_pts + axis_text_dim_pts + axis_text_outer_margin_pts + title_margin_pts
