...
- length:
  ...
  Length of the bracket caps. Supports `rel()`. Negative values flip the
  cap direction. Defaults to `rel(1)`. May be a vector the same length
...
- linewidth:
  ...
  Inherits from `axis.ticks` in the set theme. Supports `rel()`. May be
  a vector the same length as the number of axes.
